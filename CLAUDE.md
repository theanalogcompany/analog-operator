...
- **APNs entitlement (`aps-environment`) is EAS-build-profile-driven.** Do NOT hand-set `ios.entitlements["aps-environment"]` in `app.json`. EAS auto-injects `production` for the `production` build profile (TestFlight + AppStore) and `development` for the `development` profile. Hard-coding the value in `app.json` overrides EAS and silently breaks the other profile (e.g., production-pinned in `app.json` → dev builds get prod entitlements → APNs dev pushes silently dropped). The `expo-notifications` config-plugin's optional `mode` key is similarly redundant once EAS is wired — leave it off; let the build profile own it. `app.json` only declares `UIBackgroundModes: ["remote-notification"]` and the `expo-notifications` plugin entry (no inline config). (TAC-288.)
